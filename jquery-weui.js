  ...
  var Calendar = function (params) {
      ...
      p.updateValue = function () {
          ...
          for (i = 0; i < p.value.length; i++) {
              if (/\d{4}-\d{1,2}-\d{1,2}/.test(p.value[i])) {
                var dateArr = p.value[i].split(/[- : \/]/);
                var valueDate = new Date(dateArr[0],dateArr[1]-1,dateArr[2]);
              } else {
      ...
      p.monthHTML = function (date, offset) {
          ...
          if (!p.params.minDate) {
            ...
            if (isString(p.params.minDate)) {
              var minDate = p.params.minDate;
            } else{
              var minDate = p.params.minDate.join()
            };
            var minDateArry = minDate.split(/[- : \/]/);
            var newMinDate = minDateArry.join(",");
          }

          if (!p.params.maxDate) {
            var newMaxDate = null;
          } else {
            if (isString(p.params.maxDate)) {
              var maxDate = p.params.maxDate;
            } else{
              var maxDate = p.params.maxDate.join()
            };
            var maxDateArry = maxDate.split(/[- : \/]/);
            var newMaxDate = maxDateArry.join(",");
          }
          ...
          var dayDate, currentValues = [], i, j,
              ...
              minDate = p.params.minDate ? new Date(newMinDate).getTime() : null,
              maxDate = p.params.maxDate ? new Date(newMaxDate).getTime() : null;
      ...
      p.nextMonth = function (transition) {
          ...
          if (p.params.maxDate) {
              if (nextDateTime > new Date(newMaxDate).getTime()) {
                  return p.resetMonth();
              }
          }
          p.monthsTranslate --;
      ...
      p.prevMonth = function (transition) {
          ...
          if (p.params.minDate) {
              if (prevDateTime < new Date(newMinDate).getTime()) {
                  return p.resetMonth();
              }
          }
          p.monthsTranslate ++;
      ...
      p.setYearMonth = function (year, month, transition) {
          ...
          var maxDateArr = p.params.maxDate.split(/[- : \/]/);
          var minDateArr = p.params.minDate.split(/[- : \/]/);
          if (p.params.maxDate && targetDate > new Date(maxDateArr[0],maxDateArr[1]-1,maxDateArr[2]).getTime()) {
              return false;
          }
          if (p.params.minDate && targetDate < new Date(minDateArr[0],minDateArr[1]-1,minDateArr[2]).getTime()) {
              return false;
          }
